    ...
    <el-card ref="card" class="app-base-el-card flex-1 w-full">
      ...
      <li v-show="onDrag" class="w-full h-full z-50 flex items-center justify-center">
        <div class="w-full h-full border-2 border-dashed border-blue-500 rounded-lg flex items-center justify-center">
          <span class="text-blue-500 text-lg">{{ I18nT('tools.ImageCompress.batchProcessing.dragDrop') }}</span>
        </div>
      </li>
      <el-table
        v-show="!onDrag"
      ...
      >
        ...
        <el-table-column prop="path" :label="I18nT('base.path')" min-width="200">
          <template #default="scope">
            <BatchImageTablePathCell :item="scope.row" />
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template #default="scope">
            ...
            <div v-else-if="scope.row?.result?.hasError">
